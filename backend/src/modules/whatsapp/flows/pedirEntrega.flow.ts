...
async function manejarConfirmacion(m: MensajeEntrante, sesion: Sesion): Promise<void> {
  ...
  const [viaje] = await query<{ id: string }>(
    `INSERT INTO viajes (tipo, fecha, cliente_telefono, destino_direccion, estado, notas)
     VALUES ('entrega', CURRENT_DATE, $1, $2, 'programado', 'Pedido de entrega por WhatsApp (cuenta corriente)')
     RETURNING id`,
    [to, destinoDireccion],
  );
